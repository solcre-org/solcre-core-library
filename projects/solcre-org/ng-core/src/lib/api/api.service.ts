...
export class ApiService {
	...
	public fetchData(
		...
		//Get options
		const httpOptions = {
			headers: new HttpHeaders(Object.assign(headers, customHeaders)),
			params: params,
		};

		//Do request
		return this.httpClient.get(this.config.apiUrl + uri, httpOptions).pipe(
			//Map response
			map((response: any) => {
	...
	public partialUpdateObj(
		...
		//Do request
		return this.httpClient.patch(url, obj, httpOptions).pipe(
			//Map response
			map((response: any) => {
				//Return api response model
				return this.parseSingleResponse(response);
